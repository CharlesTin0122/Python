...
def joint_orient_to_json(dady_joint: str, rite_json_path: str) -> dict:
    ...
    jnt_jo_attr = [
        "{}.jointOrient".format(jnt) for jnt in joints
    ]  # 获取骨骼jointOrient属性列表
    jnt_jo_val = [
        pm.getAttr(val) for val in jnt_jo_attr
    ]  # 获取获取骨骼jointOrient属性列表属性值向量列表
...
def json_to_joint_orient(mesh_list: list, read_json_path: str) -> dict:
    ...
    """
    skin_clusters = [
        pm.listHistory(mesh, type="skinCluster") for mesh in mesh_list
    ]  # 获取所有模型的蒙皮节点
    # 将所有蒙皮节点打开骨骼移动模式
