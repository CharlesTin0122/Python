...
def child_jnt_creater(jnt, child_jntname):
	...
	child_jnt = pm.duplicate(jnt, name=child_jntname, parentOnly=True)[0]  # 复制骨骼为子骨骼并重命名
	pm.parent(child_jnt, jnt)  # 设置子骨骼父子关系
	skin_cluster = pm.listConnections(jnt, type='skinCluster')[0]  # 获取蒙皮节点
	pm.skinCluster(skin_cluster, edit=True, addInfluence=str(child_jnt), weight=0)  # 将子骨骼添加到蒙皮节点
	pm.skinCluster(skin_cluster, edit=True, selectInfluenceVerts=jnt)  # 选择父骨骼蒙皮影响的点
	pm.skinPercent(skin_cluster, transformMoveWeights=[jnt, child_jnt])  # 传递父骨骼的蒙皮权重到子骨骼
