...
def find_hierarchy():
    ...
    sel_dag_path_fn = OpenMaya.MFnDagNode(
        sel_dag_path
    )  # 创建DagNode函数集，将sel_dag_path装入函数集
...
def create_node():
    ...
    parent_obj = parent_dag_path_fn.create(
        "transform", "parent"
    )  # 使用MFnDagNode类的方法创建节点
...
def list_attr():
    ...
    )  # 通过实例化OpenMaya.MSelectionList()创建一个选择列表对象selected
...
def add_attr():
    ...
    attr_obj = fn_num_attr.create(
        "myNewAttr", "MNA", OpenMaya.MFnNumericData.kDouble, 1.0
    )
...
def double_array_attr():
    ...
    skin_cluster = (
        OpenMaya.MObject()
    )  # 实例化OpenMaya.MObject()创建一个MObject对象skin_cluster，用于存储皮肤集群节点。
    selected.getDependNode(0, skin_cluster)  # 将选择列表中的第一个节点作为依赖节点获取并存储到skin_cluster中

    skin_fn_node = OpenMaya.MFnDependencyNode(
        skin_cluster
    )  # 创建一个依赖节点函数集对象skin_fn_node，用于操作依赖节点
    skin_plug = skin_fn_node.findPlug("ptw")  # 找到名为"ptw"的属性接口skin_plug
    assert isinstance(
        skin_plug, OpenMaya.MPlug
    )  # 断言属性接口变量skin_plug是OpenMaya.MPlug对象，以便在编码时获得自动补全功能
    ...
    )  # 断言weights变量是OpenMaya.MDoubleArray对象，以便获得自动补全功能
...
def matrix_array():
    ...
    joint_fn_node = OpenMaya.MFnDependencyNode(
        joint_obj
    )  # 创建一个依赖节点函数集对象joint_fn_node，用于操作骨骼节点。
    ...
    fn_matrix_data = OpenMaya.MFnMatrixData(
        matrix_obj
    )  # 创建一个矩阵数据函数集对象fn_matrix_data，用于操作矩阵数据。
    ...
    skin_fn_node = OpenMaya.MFnDependencyNode(
        skin_obj
    )  # 创建一个依赖节点函数集对象skin_fn_node，用于操作蒙皮节点。
    skin_plug = skin_fn_node.findPlug(
        "bindPreMatrix"
    )  # 找到名为"bindPreMatrix"的属性接口skin_plug。
    assert isinstance(skin_plug, OpenMaya.MPlug)  # 断言属性接口变量
    plug_0 = skin_plug.elementByLogicalIndex(
        1
    )  # 获取属性接口的第二个子接口，即数组的第二个元素，对应“joint2”的蒙皮。
    """设置绑定姿态矩阵为骨骼逆矩阵"""
    ...
    matrix_obj = fn_matrix_data.create(
        matrix.inverse()
    )  # 创建一个新的矩阵对象matrix_obj，其中矩阵值为原始矩阵的逆矩阵。
...
def connect_attr():
    ...
    color_fn_dag_node.create(
        "blendColors", "blendColorsA"
    )  # 创建blendColors节点名为blendColorsA
...
def split_target():
    ...
    OpenMaya.MGlobal.executeCommand(
        "findRelatedSkinCluster " + base_path.fullPathName(), skin_cluster
    )
    ...
    for jnt_id in range(joint_length):
        ...
        duplicate_obj = []
        OpenMaya.MGlobal.executeCommand(
            "duplicate " + target_path.fullPathName(), duplicate_obj
        )
        ...
        for vtx_id in range(target_points.length()):
            split_point.set(
                base_points[vtx_id]
                + offset[vtx_id] * weights[vtx_id * joint_path.length() + jnt_id],
                vtx_id,
            )
