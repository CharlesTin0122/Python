...
def CreateMaterials(pSdkManager, pMesh):
    colors = (
    FbxDouble3(0.0, 0.0, 0.0), FbxDouble3(0.0, 1.0, 1.0), FbxDouble3(0.0, 1.0, 0.0), FbxDouble3(1.0, 1.0, 1.0),
...
def MapVertexCacheOnTriangle(pSdkManager, pTriangle, pSampleFileName):
    ...
    lRelativePCFileName = FbxPathUtils.GetRelativeFilePath(
        (FbxPathUtils.GetFolderName(lFBXAbsolutePath.Buffer()) + "/").Buffer(), lAbsolutePCFileName.Buffer())
...
def AnimateVertexCacheOnTriangleDoubleVertex(pTriangle, pFrameRate):
    ...
    if gExportVertexCacheMCFormat:
        lRet = lCache.OpenFileForWrite(FbxCache.EMCFileCount.eMCOneFile, pFrameRate, pTriangle.GetName(),
                                       FbxCache.EMCBinaryFormat.eMCX)
    else:
...
def AnimateVertexCacheOnTriangleInt32(pTriangle, pFrameRate):
    ...
    if gExportVertexCacheMCFormat:
        lRet = lCache.OpenFileForWrite(FbxCache.EMCFileCount.eMCOneFile, pFrameRate, pTriangle.GetName(),
                                       FbxCache.EMCBinaryFormat.eMCX, FbxCache.kInt32Array)
