        ...
        with temlate:
            with pm.columnLayout(rowSpacing=5, adj=True):
                ...
                with pm.frameLayout(label="Import multiple FBX"):
                    with pm.columnLayout(adj=1):
                        pm.button(label="Load All fbx", c=load)
                    with pm.scrollLayout(w=200, h=150, bgc=(0.5, 0.5, 0.5)) as sly:
                        pm.text("fbx Name:")
                    with pm.rowLayout(
                        numberOfColumns=3,
...
def impAnim(*args):
    ...
    for i in range(len(jointSl)):
        pm.parentConstraint(
            jointSl[i], ctrlSl[i], mo=True, skipTranslate=["x", "y", "z"]
        )
...
def load(*args):
    ...
    for i in range(len(fbxList)):
        fbxName = fbxList[i].split("/")[-1]
        with sly:
            pm.text(label=fbxName)
...
def pasteMirPose(*args):
    ...
    for key, value in mirData.items():
        if ("IK" in key or "Pole" in key) and (
            ...
            or "Neck" in key
            or "Head" in key
        ) and ("rotateX" in key or "rotateY" in key):
            pm.setAttr(key, value * -1)
        else:
            pm.setAttr(key, value)
